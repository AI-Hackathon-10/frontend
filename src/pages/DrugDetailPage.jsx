...
export default function DrugDetailPage() {
  ...
  return (
    <div className="page page--drug-detail">
      <PageHeader eyebrow="의약품 상세" title={drug.name} description={`${drug.englishName} · 예시 데이터`} backTo="/search/results" action={<Badge tone="soft">공식정보 확인 전</Badge>} />
      <section className="drug-hero-card"><div className="drug-hero-card__visual"><span className="drug-hero-card__label">식별 이미지</span><div className="drug-scene"><span className="drug-scene__grid" /><PillIllustration imprint={drug.imprintFront} size="large" variant={drug.imageVariant} /><PillIllustration imprint={drug.imprintBack} size="medium" variant={drug.imageVariant} /></div><span className="drug-hero-card__caption">앞면 {drug.imprintFront} · 뒷면 {drug.imprintBack}</span></div><div className="drug-hero-card__summary"><Badge tone="blue">{drug.classification}</Badge><p className="drug-hero-card__name">{drug.name}</p><p>{drug.effect}</p><div className="drug-hero-card__meta"><span><Icon name="shield" size={15} /> 예시 데이터</span><span><Icon name="check" size={15} /> 정보 항목 확인</span></div></div></section>
